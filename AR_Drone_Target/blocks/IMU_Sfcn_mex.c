 ...
 *   C source code generated on : 18-Sep-2019 16:02:12
...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
    ...
    if (isSimulationTarget) {
       ...
       */
      ssRegMdlInfo(S, "IMU_Sfcn_mex_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0,
                   0, ssGetPath(S));

      /*
       * Register reserved identifier for freeing PWork for SimulationTarget
       */
      ssRegMdlInfo(S, "IMU_Sfcn_mex_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0,
                   ssGetPath(S));
...
static void mdlStart(SimStruct *S)
{
  if (isDWorkNeeded(S)) {
    ...
    /*
     * Configure the pwork 0 (__y1BUS)
     */
    {
      IMU_Packets* __y1BUS = (IMU_Packets*)calloc(sizeof(IMU_Packets),
...
static void mdlTerminate(SimStruct *S)
{
  if (isDWorkNeeded(S)) {
    ...
    /*
     * Free memory for the pwork 0 (__y1BUS)
     */
    {
      IMU_Packets* __y1BUS = (IMU_Packets*)ssGetPWorkValue(S, 0);
      if (__y1BUS!=NULL) {
...
static void CheckDataTypes(SimStruct *S)
{
  /* Verify Bus/StructType 'IMU_Packets', specification */
  {
    ...
    if (status==-1) {
      ssSetErrorStatus(S,
                       "Unexpected error when checking the validity of the Simulink Bus/StructType 'IMU_Packets'");
    } else if (status==0) {
      ssSetErrorStatus(S,
                       "The Simulink Bus/StructType 'IMU_Packets' definition has changed since the S-Function was generated");
    }
  }
}
